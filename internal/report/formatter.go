...
func (f *Formatter) Format(dr DailyReport) string {
	...
	if len(dr.Sessions) == 0 && len(dr.GitCommits) == 0 {
		b.WriteString("\nNo activity recorded for this day.\n")
		return b.String()
	}

	if len(dr.Sessions) == 0 {
		// Git commits present but no tracked sessions
	...
	if len(dr.GitCommits) > 0 {
		b.WriteString("\n## Git commits\n")
		for _, rc := range dr.GitCommits {
			fmt.Fprintf(&b, "\n### %s\n\n", rc.RepoName)
			b.WriteString("| Time | Hash | Commit |\n")
			b.WriteString("|------|------|--------|\n")
			for _, c := range rc.Commits {
				localTime := c.Timestamp.In(time.Local).Format("15:04")
				fmt.Fprintf(&b, "| %s | `%s` | %s |\n", localTime, c.Hash, c.Subject)
			}
		}
	}
